...
const palette: Theme['palette'] = {
  ...
  muted: '#9a8a76',
  blue: '#4f9fd8',
  blueLight: 'rgba(142,202,230,0.18)',
  orange: '#ff8fa3',
  orangeLight: 'rgba(255,143,163,0.16)',
  green: '#4fb894',
  greenLight: 'rgba(127,216,190,0.20)',
  gold: '#ffd166',
  red: '#e85d5d',
  navy: '#4a3b2f',
  paper: '#ffffff',
  paperWarm: '#fdf6ea',
  paperBase: '#fdf6ea',
  line: 'rgba(74,59,47,0.14)',
  lineOrange: 'rgba(255,143,163,0.40)',
...
// 贴纸影：柔和双层扩散影，白边由 Paper 的 outline 承担。
const paperShadow = (lift: number) =>
  `0 ${4 + 4 * lift}px 0 rgba(74,59,47,${0.06 + 0.04 * lift}),0 ${10 + 12 * lift}px ${22 + 16 * lift}px rgba(74,59,47,${0.10 + 0.06 * lift})`;
...
const Paper: Theme['Paper'] = ({children, style, lift = 0}) =>
  <div style={{position: 'absolute', backgroundColor: palette.paper, borderRadius: aesthetic.paperRadius, outline: '5px solid #ffffff', color: palette.ink, boxShadow: paperShadow(lift), ...style}}>{children}</div>;
...
const SubtitleChrome: Theme['SubtitleChrome'] = ({mode, children}) =>
  <div style={{position: 'absolute', left: 0, right: 0, bottom: 42, zIndex: 200, display: 'flex', justifyContent: 'center'}}>
    <div style={{position: 'relative'}}>
      <Tape x={-50} y={-20} w={130} rot={-8} color="rgba(142,202,230,.75)" />
