...
const Background: React.FC = () => {
  ...
  return <>
    ...
    {/* 黄色圆角方块：卡片下方与字幕条上沿之间 */}
    <div style={{position: 'absolute', left: dw * 0.583, top: dh * 0.732, width: dw * 0.104, height: dw * 0.104, borderRadius: dw * 0.025, background: palette.gold, opacity: 0.9, transform: 'rotate(14deg)'}} />
    {/* 孟菲斯点缀：按画布比例锁定锚点 */}
    <svg width={dw} height={dh} style={{position: 'absolute', inset: 0}} fill="none">
      ...
      {Array.from({length: 5}).map((_, r) => Array.from({length: 6}).map((_, c) =>
        <circle key={`${r}-${c}`} cx={dw * 0.056 + c * 26} cy={dh * 0.52 + r * 26} r={4.5} fill={palette.green} opacity={0.5} />))}
    </svg>
  </>;
};

const Paper: Theme['Paper'] = ({children, style, lift = 0, borderColor}) =>
  <div style={{position: 'absolute', backgroundColor: palette.paper, border: `${aesthetic.paperOutline}px solid ${borderColor || palette.ink}`, borderRadius: aesthetic.paperRadius, color: palette.ink, boxShadow: `${paperShadow(lift)}, inset 0 0 0 2px ${palette.white}`, ...style}}>{children}</div>;
...
const SubtitleChrome: Theme['SubtitleChrome'] = ({mode, children}) =>
  <div style={{position: 'absolute', left: 0, right: 0, bottom: 0, height: 128, background: palette.ink, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
    ...
    <div style={{width: 10, height: 44, background: palette.green, borderRadius: 6, marginLeft: 26}} />
  </div>;
