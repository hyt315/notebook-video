...
const AESTHETIC={subtitleSafeWidth:MODE.safe,paperRadius:12,paperOutline:1.6,textureOpacity:.026,gridOpacity:.09,gradeWarmth:.055,gradeVignette:.06};
...
const Subtitle=()=>{const f=useRawCurrentFrame(),lead=Math.round(FPS*.06),hold=Math.round(FPS*.267);let cue=captions.find((c:any)=>f>=c.startFrame-lead&&f<=c.endFrame+hold);if(!cue)cue=[...captions].reverse().find((c:any)=>c.startFrame<=f);const full=cue?cleanTail(cue.text):'';const shown=cue?cleanTail(cue.words.filter((w:any)=>w.startFrame<=f+lead).map((w:any)=>w.part).join('')):'';return <div style={{position:'absolute',left:MODE.subMar,right:MODE.subMar,bottom:MODE.subBottom,height:MODE.subH,zIndex:200,background:C.paper,clipPath:'polygon(0 8%,3% 3%,6% 7%,9% 2%,12% 6%,15% 1%,18% 5%,21% 2%,24% 7%,27% 2%,30% 5%,33% 1%,36% 6%,39% 2%,42% 7%,45% 1%,48% 5%,51% 2%,54% 7%,57% 2%,60% 5%,63% 1%,66% 6%,69% 2%,72% 7%,75% 2%,78% 5%,81% 1%,84% 6%,87% 2%,90% 7%,93% 2%,96% 5%,100% 3%,100% 95%,97% 99%,94% 94%,91% 100%,88% 95%,85% 99%,82% 94%,79% 99%,76% 95%,73% 100%,70% 94%,67% 99%,64% 95%,61% 100%,58% 94%,55% 99%,52% 95%,49% 100%,46% 94%,43% 99%,40% 95%,37% 100%,34% 94%,31% 99%,28% 95%,25% 100%,22% 94%,19% 99%,16% 95%,13% 100%,10% 94%,7% 99%,4% 95%,0 98%)',filter:'drop-shadow(7px 8px 0 rgba(92,75,55,.22)) drop-shadow(0 14px 20px rgba(48,35,22,.14))',display:'grid',placeItems:'center',padding:`0 ${(MODE.designW-MODE.subMar*2-MODE.safe)/2}px`}}>
  <div style={{position:'absolute',left:28,top:28,bottom:28,width:10,borderRadius:4,background:C.orange}}/>
  <div style={{width:AESTHETIC.subtitleSafeWidth,display:'grid',placeItems:'center',fontFamily:'Smiley',fontSize:TYPE.subtitle,lineHeight:1.18,letterSpacing:1.2,whiteSpace:'nowrap'}}><span style={{position:'relative'}}><span style={{visibility:'hidden'}}>{full}</span><span style={{position:'absolute',left:0,top:0}}>{shown}</span></span></div>
</div>};

const Chrome=()=>{const f=q(useCurrentFrame()),stage=f<216?0:f<396?1:f<600?2:3,starts=[0,216,396,600],local=f-starts[stage],p=pop(local,-8),titles=COPY.chapterTitles;return <>
  <Paper style={{left:isPortrait?40:92,top:isPortrait?80:74,width:isPortrait?330:392,height:isPortrait?76:82,zIndex:150,display:'flex',opacity:p,transform:`translateY(${14*(1-p)}px) scale(${.96+.04*p})`}}><div style={{width:isPortrait?60:72,background:C.orange,color:C.white,display:'grid',placeItems:'center',fontFamily:'Smiley',fontSize:isPortrait?28:31}}>{String(stage+1).padStart(2,'0')}</div><div style={{padding:isPortrait?'8px 14px':'10px 16px'}}><div style={{fontSize:TYPE.microS,color:C.blue,fontWeight:700,letterSpacing:2}}>{COPY.chromeKicker}</div><div style={{fontSize:TYPE.titleS,fontWeight:700,marginTop:3}}>{titles[stage]}</div></div></Paper>
  <div style={{position:'absolute',right:isPortrait?40:88,top:isPortrait?76:70,zIndex:140,textAlign:'right'}}><div style={{fontSize:TYPE.labelM,fontWeight:700,letterSpacing:4,color:C.blue}}>{COPY.header}</div><div style={{fontSize:TYPE.microL,marginTop:7,color:C.muted}}>{COPY.headerSub}</div></div>
</>};
...
const SceneWorld=()=>{const f=q(useCurrentFrame()),l=f,opacity=stageFade(f,-5,216);const dim=ease(l,58,92,1,.42);return <div style={{position:'absolute',inset:0,opacity}}>
  ...
  <div style={{position:'absolute',left:600,top:200,width:1160,height:600,zIndex:72,opacity:pop(l,55),transform:`scale(${.86+.14*pop(l,55)})`,transformOrigin:'50% 50%'}}>
    ...
    {NODES.map((n,i)=><div key={i} style={{position:'absolute',left:n.x-38,top:n.y-4,width:76,textAlign:'center',fontFamily:'Smiley',fontSize:TYPE.bodyM,color:n.c,opacity:ease(l,84+i*9,104+i*9)}}>{n.label}</div>)}
...
const SceneContribute=()=>{const f=q(useCurrentFrame()),l=f-216,opacity=stageFade(f,211,396);const t=ease(l,44,104);const merge=ease(l,96,124);const cardX=180+560*Math.min(1,t/0.72);const steps=['读懂项目规则','建立最小改动','提交你的 PR','通过 CI 与评审'];return <div style={{position:'absolute',inset:0,opacity}}>
...
const SceneShip=()=>{const f=q(useCurrentFrame()),l=f-396,opacity=stageFade(f,391,600);const ver=ease(l,150,180);const rocket=ease(l,150,192);const g1=ease(l,120,150),g2=ease(l,132,162),g3=ease(l,144,174);return <div style={{position:'absolute',inset:0,opacity}}>
...
const SceneSkills=()=>{const f=q(useCurrentFrame()),l=f-600,opacity=ease(f,595,610);const cta=ease(l,180,214);return <div style={{position:'absolute',inset:0,opacity}}>
...
const StepRailP:React.FC<{active:number;l:number;start:number}>=({active,l,start})=>{const steps=[['01','参与',C.orange],['02','发布',C.blue],['03','运营',C.green]] as const;return <div style={{position:'absolute',left:70,top:190,width:940,height:68,zIndex:78,display:'flex',gap:20,justifyContent:'center'}}>{steps.map((s,i)=>{const on=i===active,p=pop(l,start+i*7);return <div key={s[0]} style={{width:300,height:64,display:'flex',alignItems:'center',gap:12,padding:'0 14px',background:on?C.paper:'#efe3ca',border:`${on?3:2}px solid ${on?s[2]:'#c7b696'}`,borderRadius:12,boxShadow:on?'5px 7px 0 rgba(63,49,35,.2)':'none',opacity:p,transform:`translateY(${14*(1-p)}px) scale(${on?1:.96})`}}><span style={{width:32,height:32,borderRadius:99,background:on?s[2]:'#c7b696',color:C.white,display:'grid',placeItems:'center',fontFamily:'Smiley',fontSize:19}}>{s[0]}</span><span style={{fontFamily:'Smiley',fontSize:TYPE.titleXS,color:on?s[2]:C.muted}}>{s[1]}</span></div>;})}</div>;};

const SceneWorldP=()=>{const f=q(useCurrentFrame()),l=f,opacity=stageFade(f,-5,216);const dim=ease(l,58,92,1,.42);return <div style={{position:'absolute',inset:0,opacity}}>
  <Paper style={{left:60,top:200,width:960,height:230,zIndex:60,padding:'22px 26px',borderColor:C.muted,opacity:dim,transform:`translateY(${26*(1-pop(l,5))}px) scale(${.94+.06*pop(l,5)})`}}>
    <div style={{fontFamily:'Smiley',fontSize:TYPE.displayS,color:C.muted}}>本地硬盘</div>
    <div style={{fontSize:TYPE.bodyM,fontWeight:700,marginTop:8,color:C.muted}}>代码只躺在你电脑里</div>
    <div style={{position:'absolute',left:26,right:26,top:118,bottom:20}}>{[0,1,2,3].map(i=><div key={i} style={{height:13,margin:'10px 0',width:`${72-i*9}%`,background:'#cabfa8',borderRadius:4,opacity:pop(l,14+i*6)}}/>)}</div>
  </Paper>
  <div style={{position:'absolute',left:516,top:452,fontFamily:'Smiley',fontSize:62,color:C.orange,zIndex:75,opacity:ease(l,48,68),transform:`translateY(${-10*(1-ease(l,48,68))}px)`}}>↓</div>
  <div style={{position:'absolute',left:80,top:540,width:1160,height:600,zIndex:72,transform:'scale(0.776)',transformOrigin:'left top',opacity:pop(l,55)}}>
    <svg width={1160} height={600} viewBox="0 0 1160 600" style={{overflow:'visible'}}>
      <circle cx={580} cy={300} r={140} fill="#e9f1fc" stroke={C.blue} strokeWidth={4}/>
      <ellipse cx={580} cy={300} rx={56} ry={140} fill="none" stroke={C.blue} strokeWidth={2} opacity={.45}/>
      <ellipse cx={580} cy={300} rx={110} ry={140} fill="none" stroke={C.blue} strokeWidth={2} opacity={.3}/>
  ...
  <div style={{position:'absolute',left:860,top:1010,zIndex:96,opacity:ease(l,96,120),transform:`translateY(${18*(1-ease(l,96,120))}px)`}}><Mascot size={105} f={l} wave/></div>
  <Paper style={{left:50,top:1130,width:980,height:86,zIndex:85,display:'grid',placeItems:'center',borderColor:C.green,opacity:ease(l,128,150),transform:`translateY(${18*(1-pop(l,128))}px)`}}><div style={{fontSize:TYPE.titleL,fontWeight:700}}>全世界的开发者，<span style={{color:C.green}}>一起造软件</span></div></Paper>
</div>};

const SceneContributeP=()=>{const f=q(useCurrentFrame()),l=f-216,opacity=stageFade(f,211,396);const t=ease(l,44,104);const merge=ease(l,96,124);const cardX=80+520*Math.min(1,t/0.72);const steps=['读懂项目规则','建立最小改动','提交你的 PR','通过 CI 与评审'];return <div style={{position:'absolute',inset:0,opacity}}>
  ...
  <svg width={1080} height={1440} style={{position:'absolute',inset:0,zIndex:55}}>
    ...
    <text x="80" y="612" fill={C.blue} fontFamily="Smiley" fontSize="30">main 主干</text>
    <text x="80" y="802" fill={C.orange} fontFamily="Smiley" fontSize="30">你的分支</text>
    <circle cx={732} cy={648} r={13} fill={merge>.4?C.green:'#c9b99e'} opacity={ease(l,90,110)}/>
  </svg>
  <Paper lift={Math.sin(Math.PI*t)*.5} style={{left:cardX,top:700,width:210,height:96,zIndex:96,padding:'16px 18px',borderColor:C.orange,opacity:ease(l,40,58)*ease(l,120,132,1,0),transform:`translateY(${-90*Math.sin(Math.PI*t)*(cardX<400?1:.4)}px)`}}><div style={{fontFamily:'Smiley',fontSize:TYPE.titleS,color:C.orange}}>你的修改</div><div style={{fontSize:TYPE.labelL,fontWeight:700,marginTop:6,color:C.muted}}>commit</div></Paper>
  <Paper lift={.4} style={{left:540,top:520,width:250,height:108,zIndex:94,padding:'16px 20px',borderColor:C.green,opacity:merge,transform:`scale(${.7+.3*pop(l,100)})`}}><div style={{display:'flex',alignItems:'center',gap:12}}><span style={{fontFamily:'Smiley',fontSize:TYPE.titleM,color:C.green}}>Pull Request</span></div><div style={{fontSize:TYPE.labelL,fontWeight:700,marginTop:8,color:C.muted}}>把改动交给项目</div><div style={{position:'absolute',right:16,top:16,transform:`scale(${pop(l,120)})`}}><CheckBadge size={30}/></div></Paper>
  <Paper style={{left:60,top:840,width:960,height:420,zIndex:76,padding:'26px 30px',borderColor:C.blue,transform:`translateX(${36*(1-pop(l,10))}px)`,opacity:pop(l,10)}}>
    <div style={{fontFamily:'Smiley',fontSize:TYPE.displayS,color:C.blue}}>第一步：参与开源</div>
    <div style={{position:'absolute',left:30,right:30,top:104,bottom:112}}>{steps.map((s,i)=>{const p=pop(l,40+i*20);return <div key={s} style={{display:'flex',alignItems:'center',gap:16,height:48,marginBottom:10,opacity:p,transform:`translateX(${24*(1-p)}px)`}}><span style={{width:40,height:40,borderRadius:99,background:[C.orange,C.blue,C.green,C.gold][i],color:C.white,display:'grid',placeItems:'center',fontFamily:'Smiley',fontSize:20}}>{i+1}</span><span style={{fontSize:TYPE.titleXS,fontWeight:700}}>{s}</span></div>;})}</div>
    <div style={{position:'absolute',left:30,right:30,bottom:30,height:44,background:'#eadcc1',border:`2px solid ${C.green}`,borderRadius:10,display:'grid',placeItems:'center',fontFamily:'Smiley',fontSize:TYPE.bodyL,color:C.green,letterSpacing:1,opacity:ease(l,110,130)}}>github-oss-contribute</div>
  </Paper>
</div>};

const SceneShipP=()=>{const f=q(useCurrentFrame()),l=f-396,opacity=stageFade(f,391,600);const ver=ease(l,150,180);const rocket=ease(l,150,192);const g1=ease(l,120,150),g2=ease(l,132,162),g3=ease(l,144,174);return <div style={{position:'absolute',inset:0,opacity}}>
  <Paper style={{left:60,top:200,width:960,height:330,zIndex:70,padding:'24px 28px',borderColor:C.orange,transform:`translateY(${26*(1-pop(l,6))}px) scale(${.95+.05*pop(l,6)})`,opacity:pop(l,6)}}>
    <div style={{fontFamily:'Smiley',fontSize:TYPE.displayS,color:C.orange}}>第二步：发布</div>
    <div style={{fontSize:TYPE.titleXS,fontWeight:700,marginTop:8,color:C.muted}}>整理成专业开源仓库</div>
    <div style={{position:'absolute',left:150,top:124,width:280,height:136,background:'#efe3ca',border:`3px solid ${C.orange}`,borderRadius:'10px 10px 0 0'}}><div style={{position:'absolute',left:-1,right:-1,top:-24,height:24,background:C.orange,clipPath:'polygon(0 100%,10% 0,90% 0,100% 100%)'}}/><div style={{padding:'12px'}}>{['README','LICENSE','CI'].map((m,i)=><div key={m} style={{height:32,margin:'5px 0',background:C.paper,border:`2px solid ${[C.blue,C.green,C.gold][i]}`,borderRadius:6,display:'grid',placeItems:'center',fontFamily:'Smiley',fontSize:TYPE.bodyM,opacity:ease(l,44+i*22,64+i*22)}}>{m}</div>)}</div></div>
    <div style={{position:'absolute',left:34,right:34,bottom:20,height:38,background:'#eadcc1',border:`2px solid ${C.orange}`,borderRadius:10,display:'grid',placeItems:'center',fontFamily:'Smiley',fontSize:TYPE.bodyM,color:C.orange,opacity:ease(l,96,116)}}>github-oss-prep</div>
  </Paper>
  <Module name="README" color={C.blue} l={l} start={40} fromX={940} toX={230} toY={336} top={470}/>
  <Module name="LICENSE" color={C.green} l={l} start={62} fromX={940} toX={230} toY={373} top={470}/>
  <Paper style={{left:60,top:560,width:960,height:600,zIndex:72,padding:'24px 28px',borderColor:C.navy,transform:`translateY(${26*(1-pop(l,72))}px) scale(${.95+.05*pop(l,72)})`,opacity:pop(l,72)}}>
    <div style={{fontFamily:'Smiley',fontSize:TYPE.displayS,color:C.navy}}>第三步：运营</div>
    <div style={{fontSize:TYPE.titleXS,fontWeight:700,marginTop:8,color:C.muted}}>分流 · 审查 · 按时发版</div>
    ...
    <div style={{position:'absolute',left:28,top:360,display:'flex',alignItems:'center',gap:20,opacity:ease(l,150,172)}}><span style={{fontFamily:'Smiley',fontSize:TYPE.titleL,color:C.muted}}>v1.0</span><span style={{fontFamily:'Smiley',fontSize:TYPE.titleL,color:C.orange}}>→</span><span style={{fontFamily:'Smiley',fontSize:TYPE.displayS,color:C.green,transform:`scale(${.7+.3*pop(l,152)})`,display:'inline-block'}}>v1.1</span></div>
    <svg width={120} height={160} style={{position:'absolute',right:40,bottom:90,transform:`translateY(${-150*rocket}px)`,opacity:ease(l,148,164)}} viewBox="0 0 120 160"><path d="M60 6 C86 40 86 88 60 120 C34 88 34 40 60 6 Z" fill={C.paper} stroke={C.red} strokeWidth={4}/><circle cx={60} cy={54} r={13} fill="#e9f1fc" stroke={C.blue} strokeWidth={3}/><path d="M40 96 L24 128 L44 116 Z" fill={C.orange} stroke={C.red} strokeWidth={3} strokeLinejoin="round"/><path d="M80 96 L96 128 L76 116 Z" fill={C.orange} stroke={C.red} strokeWidth={3} strokeLinejoin="round"/><path d="M50 120 q10 24 20 0" fill={C.gold} opacity={.9}/></svg>
    <div style={{position:'absolute',left:34,right:34,bottom:24,height:42,background:'#eadcc1',border:`2px solid ${C.navy}`,borderRadius:10,display:'grid',placeItems:'center',fontFamily:'Smiley',fontSize:TYPE.bodyM,color:C.navy,opacity:ease(l,158,178)}}>github-oss-ops</div>
  </Paper>
</div>};

const SceneSkillsP=()=>{const f=q(useCurrentFrame()),l=f-600,opacity=ease(f,595,610);const cta=ease(l,180,214);return <div style={{position:'absolute',inset:0,opacity}}>
  <div style={{position:'absolute',left:470,top:205,zIndex:75,textAlign:'center',opacity:pop(l,-10),transform:`scale(${.7+.3*pop(l,-10)})`}}><Mascot size={110} f={l} wave/><div style={{fontFamily:'Smiley',fontSize:TYPE.titleM,color:C.navy,marginTop:2}}>AI AGENT</div></div>
  {SKILLS.map((s,i)=>{const p=pop(l,10+i*16);const y=340+i*265;return <Paper key={s[0]} lift={.25} style={{left:60,top:y,width:960,height:250,zIndex:94,padding:'24px 28px',borderColor:s[3],opacity:p,transform:`translateY(${34*(1-p)}px) scale(${.92+.08*p})`}}>
    ...
    <div style={{position:'absolute',left:104,top:34,fontFamily:'Smiley',fontSize:TYPE.titleM,color:s[3]}}>{s[0]}</div>
...
const FinalDemo=()=>{const f=q(useCurrentFrame());if(isPortrait)return <>{f<216&&<SceneWorldP/>}{f>=216&&f<396&&<SceneContributeP/>}{f>=396&&f<600&&<SceneShipP/>}{f>=600&&<SceneSkillsP/>}</>;return <>{f<216&&<SceneWorld/>}{f>=216&&f<396&&<SceneContribute/>}{f>=396&&f<600&&<SceneShip/>}{f>=600&&<SceneSkills/>}</>};
...
const Sound=()=> <>
  ...
  {[5,221,401,605].map(f=><Sequence key={`r${f}`} from={deliveryFrame(f)} layout="none"><Audio src={staticFile('sfx/paper-rustle.wav')} volume={.13}/></Sequence>)}
  {[60,311,546].map(f=><Sequence key={`w${f}`} from={deliveryFrame(f)} layout="none"><Audio src={staticFile('sfx/data-whoosh.wav')} volume={.12}/></Sequence>)}
  {[20,90,246,311,436,458,480,630,646,662].map(f=><Sequence key={`t${f}`} from={deliveryFrame(f)} layout="none"><Audio src={staticFile('sfx/paper-tap.wav')} volume={.15}/></Sequence>)}
  {[150,336,559,794].map(f=><Sequence key={`c${f}`} from={deliveryFrame(f)} layout="none"><Audio src={staticFile('sfx/chime.wav')} volume={.18}/></Sequence>)}
</>;
