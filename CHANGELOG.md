...
- **时间轴跟着配音走**：`build-semantic-captions`（常规流程，一行 = 一条 cue）→ `resolve-shots.py` 重算分镜帧。
  总长 **1150 → 1453 帧**（注：这一段描述的是**外部验证工程** demo-video，技能模板自身仍是 1150 帧）（+303 帧 / +10.1s）**；各镜帧位（旧 → 新）：
...
**第八轮：`NetworkGraph` 的入场也封顶（与 `TreeView` 同一套公式）**

`network.tsx` 原来还是 `startAt + 4 + i*5`（连线）与 `startAt + 10 + i*5`（节点）——和 `TreeView`
修之前同一个毛病：**元素一多，入场就线性拖长**。改成同一套公式
`间隔 = min(5, 窗口 / (数量 − 1))`（连线窗口 25 帧、节点窗口 25 帧）。

**封顶前后（最后一个元素"完全就位"的帧号，纯算术）**

| 元素数 | 连线 旧 → 新 | 节点 旧 → 新 |
|---|---|---|
| 1–6 | 30/35/40/45/50/**55** → 同左（**逐一相同**） | 34/39/44/49/54/**59** → 同左（**逐一相同**） |
| 8 | 65 → **55** | 69 → **59** |
| 10 | 75 → **55** | 79 → **59** |
| 20 | 125 → **55** | 129 → **59** |
| 100 | 525 → **55** | 529 → **59** |

**小规模行为不变**：≤6 个元素时间隔仍是 5 帧，所以**接触表 ⑯ 页（6 节点 / 6 连线）的输出逐字节不变**
——`NetworkGraph` 目前唯一进入画面的用法就是这一处（另一处是 `diag-upgrade` 页②的夹具，8 节点/9 连线，
